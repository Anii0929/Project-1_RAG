...
class TestRAGSystem:
    ...
    def test_init(self, mock_config, mock_dependencies):
        ...
        mock_dependencies['vector_store'].assert_called_once_with("./test_chroma", "test-model", 5)
        mock_dependencies['ai_generator'].assert_called_once_with("test-api-key", "claude-3-5-sonnet-20241022")
    ...
    def test_add_course_folder_success(self, rag_system):
        ...
        with patch('rag_system.os.path.exists', return_value=True), \
             patch('rag_system.os.listdir', return_value=['course1.txt', 'course2.pdf', 'other.xyz']), \
    ...
    def test_add_course_folder_skip_existing(self, rag_system):
        ...
        with patch('rag_system.os.path.exists', return_value=True), \
            ...
            rag_system.document_processor.process_course_document.return_value = (mock_course, [Mock()])
    ...
    def test_query_without_session(self, rag_system):
        ...
        # Mock AI response
        rag_system.ai_generator.generate_response.return_value = "Test response about Python"
        
        # Mock tool manager
        rag_system.tool_manager.get_tool_definitions.return_value = [{"name": "search_tool"}]
        rag_system.tool_manager.get_last_sources.return_value = [{"text": "Python Course - Lesson 1"}]
    ...
    def test_query_with_session(self, rag_system):
        ...
        rag_system.session_manager.get_conversation_history.assert_called_once_with("session_123")
...
class TestRAGSystemDiagnostics:
    ...
    def test_component_initialization_diagnostic(self):
        ...
        try:
            with patch('rag_system.DocumentProcessor') as mock_doc, \
    ...
    def test_query_flow_diagnostic(self):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            try:
                ...
                print(f"   Tool definitions retrieved: {system.tool_manager.get_tool_definitions.called}")
                print(f"   Sources retrieved: {system.tool_manager.get_last_sources.called}")
