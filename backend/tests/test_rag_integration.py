...
class TestRAGSystemInitialization:
    ...
    def test_rag_system_init_unknown_provider(self):
        ...
        with (
            ...
            with pytest.raises(
                ValueError, match="Unknown AI provider: unknown_provider"
            ):
                RAGSystem(config)
...
class TestRAGSystemQuery:
    ...
    def test_query_successful_with_tools(self):
        ...
        rag.ai_generator.generate_response.return_value = (
            "Based on the search results, MCP is a protocol for AI applications."
        )
...
class TestRAGSystemDocumentProcessing:
    ...
    def test_add_course_document_success(self):
        ...
        with (
            patch("rag_system.DocumentProcessor") as mock_proc,
            ...
            test_course = Course(
                ...
                lessons=[
                    Lesson(lesson_number=0, title="Introduction", lesson_link=None)
                ],
            )
            test_chunks = ["chunk1", "chunk2", "chunk3"]

            mock_processor_instance = Mock()
            mock_processor_instance.process_course_document.return_value = (
                test_course,
                test_chunks,
            )
            ...
            mock_processor_instance.process_course_document.assert_called_once_with(
                "test_file.txt"
            )
    ...
    def test_add_course_document_failure(self):
        ...
        with (
            ...
            mock_processor_instance = Mock()
            mock_processor_instance.process_course_document.side_effect = Exception(
                "File not found"
            )
            mock_proc.return_value = mock_processor_instance
