...
class TestQueryFailedFix:
    ...
    def test_course_listing_query_success(self):
        ...
        try:
            ...
            for query in test_queries:
                ...
                assert "query failed" not in response.lower(), f"Query '{query}' still failing: {response[:200]}..."
                assert "Error executing tool" not in response, f"Tool execution error in '{query}': {response[:200]}..."
                ...
                contains_course_info = any(keyword in response.lower() for keyword in [
                    "course", "available", "mcp", "chroma", "anthropic", "lesson"
                ])
                contains_helpful_guidance = "Please specify" in response or "Available courses:" in response
                
                assert is_substantial or contains_course_info or contains_helpful_guidance, \
                    f"Response for '{query}' lacks substance: {response}"
                
                # Should have sources (the new CourseListTool should provide sources)
                assert isinstance(sources, list)
                
                print(f"✓ Query '{query}' succeeded - Response length: {len(response)}, Sources: {len(sources)}")
    ...
    def test_specific_course_queries_still_work(self):
        ...
        try:
            ...
            for query in test_queries:
                ...
                assert "query failed" not in response.lower(), f"Content query '{query}' failed: {response[:200]}..."
                assert isinstance(response, str)
                assert len(response) > 50, f"Content response too short for '{query}': {response}"
    ...
    def test_outline_queries_still_work(self):
        ...
        try:
            ...
            for query in test_queries:
                ...
                assert "query failed" not in response.lower(), f"Outline query '{query}' failed: {response[:200]}..."
                assert "missing 1 required positional argument" not in response, f"Parameter error in '{query}': {response[:200]}..."
                assert isinstance(response, str)
    ...
    def test_all_three_tools_registered(self):
        ...
        assert len(tool_definitions) == 3, f"Expected 3 tools, got {len(tool_definitions)}: {tool_names}"
    ...
    def test_chromadb_metadata_fix_works(self):
        ...
        try:
            ...
            test_course = Course(
                ...
                lessons=[Lesson(lesson_number=0, title="Test Lesson", lesson_link=None)]
            )
