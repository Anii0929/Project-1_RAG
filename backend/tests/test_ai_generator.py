...
class TestAIGeneratorBasic:
    ...
    def test_system_prompt_content(self):
        ...
        assert (
            "NEVER answer course-related questions using your own knowledge"
            in AIGenerator.SYSTEM_PROMPT
        )
...
class TestAIGeneratorToolExecution:
    ...
    def test_generate_response_with_single_tool_use(
        ...
        with patch("ai_generator.anthropic.Anthropic") as mock_anthropic:
            ...
            assert (
                len(second_call[1]["messages"]) == 3
            )  # user + assistant tool use + user tool results
...
class TestAIGeneratorSequentialToolCalling:
    ...
    def test_max_rounds_reached(self, mock_anthropic_client, mock_tool_manager):
        ...
        with patch("ai_generator.anthropic.Anthropic") as mock_anthropic:
            ...
            generator = AIGenerator(
                "test_key", "claude-3-sonnet-20240229", max_tool_rounds=2
            )
            ...
            tools = [{"name": "search_course_content", "description": "Search tool"}]
            response = generator.generate_response(
                "Complex multi-part query", tools=tools, tool_manager=mock_tool_manager
            )
    ...
    @pytest.mark.unit
    def test_tool_error_in_sequential_flow(
        self, mock_anthropic_client, mock_tool_manager
    ):
...
class TestAIGeneratorErrorHandling:
    ...
    def test_tool_manager_none(self, mock_anthropic_client):
        ...
        with patch("ai_generator.anthropic.Anthropic") as mock_anthropic:
            ...
            response = generator.generate_response(
                "Test query", tools=tools, tool_manager=None
            )
...
class TestAIGeneratorEdgeCases:
    ...
    def test_very_long_conversation_history(self, mock_anthropic_client):
        ...
        with patch("ai_generator.anthropic.Anthropic") as mock_anthropic:
            ...
            response = generator.generate_response(
                "Test", conversation_history=long_history
            )
