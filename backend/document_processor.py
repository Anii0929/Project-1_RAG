...
class DocumentProcessor:
    ...
    def chunk_text(self, text: str) -> List[str]:
        ...
        logger.debug(
            f"Text normalized - Length after cleanup: {len(text)} chars")
            ...
            if current_chunk:
                ...
                if hasattr(self, 'chunk_overlap') and self.chunk_overlap > 0:
                    ...
                    for k in range(len(current_chunk) - 1, -1, -1):
                        sentence_len = len(
                            current_chunk[k]) + (1 if k < len(current_chunk) - 1 else 0)
    ...
    def process_course_document(self, file_path: str) -> Tuple[Course, List[CourseChunk]]:
        ...
        logger.debug(
            f"Filename: {filename}, Content length: {len(content)} characters")
        ...
        if len(lines) >= 1 and lines[0].strip():
            ...
            if title_match:
                ...
                logger.debug(
                    f"Using first line as course title: '{course_title}'")
        ...
        for i in range(1, min(len(lines), 4)):  # Check first 4 lines for metadata
            ...
            link_match = re.match(
                r'^Course Link:\s*(.+)$', line, re.IGNORECASE)
            if link_match:
        ...
        while i < len(lines):
            ...
            if lesson_match:
                ...
                lesson_link = None
                logger.info(
                    f"Found new lesson {current_lesson}: '{lesson_title}'")
        ...
        if not course_chunks and len(lines) > 2:
            ...
            if remaining_content:
                ...
                logger.debug(
                    f"Created {len(chunks)} chunks from document content")
                for chunk in chunks:
                    course_chunk = CourseChunk(
                        content=chunk,
                        course_title=course.title,
        ...
        if lesson_count > 0:
            ...
            logger.info(
                f"Lessons with links: {lessons_with_links}/{lesson_count}")
